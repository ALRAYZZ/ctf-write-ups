...
Then, using the required cURL flags to use the provided credentials with the right format, as we inspected in the web browser, and we can see we get a cookie
...
Finally, we combine and craft a full cURL command that will perform the appropriate POST request, with the correct JSON format, and we will look for the 'flag' using our acquired cookie
